...
#if DEBUG
            args = new string[] { "New", "Others" };
#endif
            if (args.Length == 2)
            {
                BusinessLogic.PollingSuppliers(System.Configuration.ConfigurationManager.ConnectionStrings["Alibaba"].ConnectionString, getFullPath(args[0]), getFullPath(args[1]));
            }
            else
                Console.WriteLine("PollSuppliers.exe [Source Path] [Destination Path]");
            stopWatch.Stop();
